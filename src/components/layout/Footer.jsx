...
const Footer = () => {
    ...
    useEffect(() => {
        ...
        const applyHoverAnimation = (element) => {
            ...
            const handleMouseEnter = () => {
                ...
                tlHover
                    .to(charsInitial, { yPercent: -100, duration: 0.6, ease: "power2.out", stagger: 0.02 })
                    .to(charsHover, { yPercent: 0, duration: 0.6, ease: "power2.out", stagger: 0.02 }, "<0.1");
            };
            ...
            const handleMouseLeave = () => {
                ...
                hoverTimelines.set(element, tlHover);
                tlHover
                    .to(charsHover, { yPercent: 100, duration: 0.6, ease: "power2.out", stagger: 0.02 })
                    .to(charsInitial, { yPercent: 0, duration: 0.6, ease: "power2.out", stagger: 0.02 }, "<0.1");
            };

            element.addEventListener("mouseenter", handleMouseEnter);
            element.addEventListener("mouseleave", handleMouseLeave);
            element._hoverHandlers = { enter: handleMouseEnter, leave: handleMouseLeave };
        };

        const navLinks = navLinksRef.current?.querySelectorAll(".footer-link-item");
        navLinks?.forEach((link) => applyHoverAnimation(link));

        const socialLinkEls = socialLinksRef.current?.querySelectorAll(".footer-link-item");
    ...
    return (
        <footer id="contact" ref={footerRef} className="bg-zinc-950 text-white pt-16 pb-12 overflow-x-hidden">
            <div className="container mx-auto px-6 lg:px-4">
                ...
                <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-6 lg:gap-8 mb-12">
                    ...
                    <div ref={ctaTextRef} className="w-full lg:w-auto max-w-full">
                        ...
                        <a href="mailto:tooopayy@gmail.com" className="hover:opacity-80 transition-opacity">
                    ...
                    <div className="relative flex items-center mt-2 lg:mt-0 shrink-0">
                        <Link to="/contact">
                        <button
                            className="group relative px-8 py-4 bg-white hover:bg-brand-blue text-black hover:text-white rounded-full font-bold flex items-center gap-4 transition-all duration-300 hover:scale-105 active:scale-95 shadow-lg border border-zinc-100 whitespace-nowrap"
                        >
                            Book a Call
                ...
                <div className="grid grid-cols-1 md:grid-cols-12 gap-10 md:gap-12 mb-20">
                    {/* Info */}
                    <div ref={infoRef} className="md:col-span-6 lg:col-span-7 order-2 md:order-1">
                    ...
                    {/* Navigation Links */}
                    <div ref={navLinksRef} className="md:col-span-3 lg:col-span-2 order-3 md:order-2">
                        <p className="text-xs font-bold uppercase tracking-widest text-zinc-500 mb-6">
                            Navigate
                        </p>
                        <ul className="flex flex-col gap-3">
                            {navigateLinks.map((link) => {
                                const targetId = (link === "Home" ? "hero" : link === "Projects" ? "projects" : link === "Contact" ? "contact" : link).toLowerCase();
                                return (
                                    <li key={link}>
                                        <LinkItem
                                            href={`#${targetId}`}
                    ...
                    {/* Social Media */}
                    <div ref={socialLinksRef} className="md:col-span-3 order-4 md:order-3">
                        <p className="text-xs font-bold uppercase tracking-widest text-zinc-500 mb-6">
                            Social Media
                        </p>
                        <ul className="flex flex-col gap-3">
                            {socialLinks.map((link) => (
                                <li key={link}>
                                    <LinkItem
                                        ...
                                        ariaLabel={`Visit our ${link} profile`}
                                    >
                                        {link}
                                    </LinkItem>
                                </li>
                ...
                <div className="flex justify-between items-end mb-4 pb-5">
                    ...
                    >
                        Toopay
                        <sup className="font-thin text-base sm:text-xl md:text-3xl lg:text-4xl absolute top-2 -right-6 sm:-right-8 md:-right-12 lg:-right-14">TM</sup>
                    </div>
