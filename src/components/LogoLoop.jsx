...
export const LogoLoop = memo(
  ({
    ...
    // FIX: dulu di-slice(0, 4) sehingga logo ke-5 dst tidak pernah tampil.
    // Sekarang semua logo yang dikirim lewat prop `logos` akan dirender.
    const displayLogos = useMemo(() => logos, [logos]);

    const [seqWidth, setSeqWidth] = useState(0);
    const [seqHeight, setSeqHeight] = useState(0);
    const [copyCount, setCopyCount] = useState(
      loop ? ANIMATION_CONFIG.MIN_COPIES : 1,
    );
    ...
    useResizeObserver(
      updateDimensions,
      [containerRef, seqRef],
      [displayLogos, gap, logoHeight, isVertical],
    );
        ...
        Array.from({ length: copyCount }, (_, copyIndex) => (
          <ul
            className={cx(
              ...
              fullWidthSequence &&
                !isVertical &&
                "w-full justify-between px-4 md:px-12 lg:px-16",
            )}
            key={`copy-${copyIndex}`}
            role="list"
          ...
          >
            {displayLogos.map((item, itemIndex) =>
